-- ========== SPIRIT HELPER TAB (С ОТДЕЛЬНЫМ ГУИ) ==========

-- Создаём кнопку в основной вкладке Spirit Helper
SpiritHelperTab:CreateButton({
    Name = "Open Spirit Helper GUI",
    Callback = function()
        -- Создаём новое окно Rayfield
        local SpiritWindow = Rayfield:CreateWindow({
            Name = "Spirit Helper",
            Icon = 0,
        ...
        local SpiritGUITab = SpiritWindow:CreateTab("helper", 4483362458)
        ...
        local function spiritCheckAndAct()
           ...
           for p, v in pairs(mDetails) do
              if spiritData.monsterProgress[p] ~= nil and v ~= spiritData.monsterProgress[p] then
                 if v == 0 and spiritData.monsterProgress[p] > 0 then
                    SpiritWindow:Notify("MONSTER IS GONE!", p .. " reset to 0", 5)
                 end
              end
           ...
           if mProgress == 3 then
              ...
              if heat and heat - (spiritData.lampHeatVal == -1 and heat or spiritData.lampHeatVal) >= 0.2 and now - spiritData.lampIncreaseTime >= 2 then
                 SpiritWindow:Notify("LAMP HEAT INCREASE!", 2)
                 spiritTP("lamp")
           ...
           if closetP and spiritData.closetProgress ~= nil then
              if closetP == 3 and spiritData.closetProgress ~= 3 then
                 SpiritWindow:Notify("CLOSET ALERT!", "Teleporting to closet!", 3)
                 spiritTP("closet")
                 spiritData.closetProgress = closetP
                 return
              end
              if closetP == 0 and spiritData.closetProgress == 3 then
                 SpiritWindow:Notify("CLOSET RESET!", "Teleporting to lamp!", 2)
                 spiritTP("lamp")
                 spiritData.lampTime = now
                 spiritData.closetProgress = closetP
                 return
           ...
           if bearDist then
              if spiritData.bearDist ~= nil and bearDist == 0 and spiritData.bearDist > 0 then
                 SpiritWindow:Notify("BEAR RESET!", "Teleporting to lamp!", 2)
                 spiritTP("lamp")
                 spiritData.lampTime = now
              ...
              if bearDist >= 6 and bearDist <= 8 and now - spiritData.bearTime >= 3 then
                 SpiritWindow:Notify("BEAR!", "Distance: " .. bearDist, 2)
                 spiritTP("bear")
           ...
           if heat then
              ...
              if needTP and now - spiritData.lampTime > 5 then
                 SpiritWindow:Notify("LAMP NEEDS " .. action .. "!", "Heat: " .. heat, 3)
                 spiritTP("lamp")
                 spiritData.lampTime = now
                 return
              end
           ...
           if radioDist then
              if spiritData.maxDist ~= nil and radioDist == 0 and spiritData.maxDist > 0 then
                 SpiritWindow:Notify("ALARM RESET!", "Teleporting to lamp!", 2)
                 spiritTP("lamp")
                 spiritData.lampTime = now
              ...
              if radioDist == 8 then
                 if not spiritData.distAt8 then
                    SpiritWindow:Notify("RADIO READY!", "Teleporting every 2 seconds", 2)
              ...
              if spiritData.distAt8 and now - spiritData.alarmTime >= 2 then
                 SpiritWindow:Notify("ALARM!", "Teleporting to alarm", 1.5)
                 spiritTP("alarm")
           ...
           if mProgress == 3 then
              ...
              if now - spiritData.monsterCooldown > 10 then
                 ...
                 SpiritWindow:Notify("MONSTER AT 3!", "Teleporting to lamp first!", 3)
                 spiritTP("lamp")
                 task.wait(3)
                 SpiritWindow:Notify("MONSTER AT 3!", "Now teleporting to bed!", 3)
                 spiritTP("bed")
                 task.wait(2)
              end
              return
        ...
        local function startSpirit()
           if spiritEnabled then 
              SpiritWindow:Notify("Spirit Helper", "Already running!", 2)
              return 
           end
           spiritEnabled = true
           ...
           SpiritWindow:Notify("Spirit Helper", "Started!", 2)
        ...
        local function stopSpirit()
           if not spiritEnabled then 
              SpiritWindow:Notify("Spirit Helper", "Not running!", 2)
              return 
           end
           spiritEnabled = false
           if spiritThread then task.cancel(spiritThread) spiritThread = nil end
           SpiritWindow:Notify("Spirit Helper", "Stopped!", 2)
        end

        local function toggleBloodmoon()
           spiritBloodmoon = not spiritBloodmoon
           SpiritWindow:Notify("Bloodmoon Mode", spiritBloodmoon and "ENABLED" or "DISABLED", 3)
        end

        -- Создаём кнопки в новом GUI
        SpiritGUITab:CreateButton({
        ...
        SpiritGUITab:CreateButton({
           ...
           Callback = function()
              local heat = spiritGetHeat()
              if not heat then 
                 SpiritWindow:Notify("Error", "Lamp not found", 2)
                 return 
              end
              SpiritWindow:Notify("Lamp Heat", "Current: " .. heat, 2)
              if heat == 0 or (heat >= 60 and heat <= 70) then
                 SpiritWindow:Notify("Manual teleport", "Teleporting to lamp", 2)
                 spiritTP("lamp")
              end
           end
        })
        ...
        SpiritWindow:Notify("Spirit Helper", "GUI Opened! Press H again to close", 3)
    end
})
